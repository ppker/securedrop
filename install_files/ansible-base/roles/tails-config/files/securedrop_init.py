...
if tails_current_version:
    try:
        needs_update = (
            len(tails_current_version) >= 2 and int(tails_current_version[1]) < tails_4_min_version
        )
    ...
    if needs_update:
        ...
        try:
            subprocess.call(
                ["torsocks", "curl", "--silent", "https://tails.boum.org/" + cert_name],
                stdout=pem_file,
                env=env,
            )
            ...
            verify_proc = subprocess.check_output(
                [
                    "openssl",
                    "verify",
                    "-no_check_time",
            ...
            if "OK" in verify_proc:
                ...
                os.setresgid(amnesia_gid, amnesia_gid, -1)
                os.setresuid(amnesia_uid, amnesia_uid, -1)
                restart_proc = subprocess.call(
                    ["systemctl", "--user", "restart", "tails-upgrade-frontend"], env=env
                )
